...
from lib.node import MESHTASTIC_ROLE, MeshNode, NodeConfig
...
def precompute_connectable_nodes(conf, node_configs):
    ...
    if total_connectable > 0:
        ...
        print("   WARNING: No connectable node pairs found! Check path loss parameters.")
    ...
    for router_idx, router_config in enumerate([n for n in node_configs if n['isRouter']]):
        ...
        if farthest_connectable:
            ...
            print("     - No nodes can receive signal from this router!")

        # Show a few example calculations for debugging
        print("     - Signal examples (closest 3 connectable):")
    ...
    if len(routers) > 1:
        ...
        if router_connectivity_pct < 100:
            print("   ⚠️  Warning: Not all routers can communicate directly!")
...
def burning_man_node_config_to_current(node_config, default_period):
    """Translate the stale PR #33 node dictionary into current NodeConfig."""
    role = MESHTASTIC_ROLE.ROUTER if node_config["isRouter"] else MESHTASTIC_ROLE.CLIENT
    message_period = node_config.get("messagePeriod", default_period)
    if message_period <= 0:
        message_period = default_period * 10

    return NodeConfig(
        node_config["nodeId"],
        Point(node_config["x"], node_config["y"], node_config["z"]),
        message_period,
        role=role,
        antenna_gain=node_config["antennaGain"],
        hop_limit=node_config["hopLimit"],
        neighbor_info=False,
        antenna_height=node_config["z"],
        tx_power_dbm=node_config["ptx"],
    )


def run_current_simulation(conf, node_configs):
    """Run Burning Man placement through the current DCR/DTP-capable simulator."""
    current_node_configs = [
        burning_man_node_config_to_current(node_config, conf.PERIOD)
        for node_config in node_configs
    ]

    random.seed(conf.SEED)
    sim = DiscreteEventSim(conf, current_node_configs, graph=None)
    ...
    print("*" * 40)
    print("Number of messages created:", results["messageSeq"])
    print("Number of packets sent:", results["sent"], "to", results["potentialReceivers"], "potential receivers")
    print("Number of collisions:", results["nrCollisions"])
    if conf.CAPTURE_COLLISION_MODEL_ENABLED:
        print("Collision reasons:", results["collisionReasons"])
    print("Number of packets sensed:", results["nrSensed"])
    print("Number of packets received:", results["nrReceived"])
    if conf.PHY_LOSS_MODEL_ENABLED:
        print("Number of packets lost by PHY model:", results["nrPhyLoss"])
    print("Average Tx air utilization:", round(results["txAirUtilizationRate"] * 100, 2), "%")
    print("Average percentage of nodes reached:", round(results["nodeReach"] * 100, 2))
    print("Percentage of received packets containing new message:", round(results["usefulness"] * 100, 2))
    if conf.DCR_ENABLED:
...
def run_burning_man_simulation(
    num_clients=100,
    enable_plotting=False,
    hop_limit=3,
    preset='SHORT_TURBO',
    debug=False,
    simtime_seconds=None,
    period_seconds=None,
    dcr=False,
    dtp=False,
    phy_loss_model=False,
    capture_collision_model=False,
):
    ...
    conf.PLOT = False
    conf.DCR_ENABLED = dcr
    conf.DTP_ENABLED = dtp
    conf.PHY_LOSS_MODEL_ENABLED = phy_loss_model
    conf.CAPTURE_COLLISION_MODEL_ENABLED = capture_collision_model
    if simtime_seconds is not None:
        conf.SIMTIME = int(simtime_seconds * conf.ONE_SECOND_INTERVAL)
    if period_seconds is not None:
        conf.PERIOD = int(period_seconds * conf.ONE_SECOND_INTERVAL)
    ...
    print(f"Mean message period: {conf.PERIOD/1000}s")
    print(f"Dynamic Coding Rate: {'enabled' if conf.DCR_ENABLED else 'disabled'}")
    print(f"Dynamic TX Power: {'enabled' if conf.DTP_ENABLED else 'disabled'}")
    print(f"PHY loss model: {'enabled' if conf.PHY_LOSS_MODEL_ENABLED else 'disabled'}")
    print(f"Capture collision model: {'enabled' if conf.CAPTURE_COLLISION_MODEL_ENABLED else 'disabled'}")
    ...
    if enable_plotting:
        connectivity_matrix, baseline_path_loss_matrix, _ = precompute_connectable_nodes(conf, node_configs)
        plot_node_locations(node_configs, conf, connectivity_matrix, baseline_path_loss_matrix)

    return run_current_simulation(conf, node_configs)
    ...
    if debug and hasattr(conf, 'BROADCAST_CSV_WRITER'):
        broadcast_csv.close()
        print("Broadcast log saved to out/broadcasts.csv")
    ...
    print("Node metrics saved to out/node_metrics.json")
    if enable_plotting:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--simtime-seconds', type=float,
                       help='Override simulation duration in seconds')
    parser.add_argument('--period-seconds', type=float,
                       help='Override mean client message period in seconds')
    parser.add_argument('--dcr', action='store_true',
                       help='Enable the Dynamic Coding Rate experiment')
    parser.add_argument('--dtp', action='store_true',
                       help='Enable the Dynamic TX Power experiment')
    parser.add_argument('--phy-loss-model', action='store_true',
                       help='Enable empirical SNR-to-payload-loss model')
    parser.add_argument('--capture-collision-model', action='store_true',
    ...
    run_burning_man_simulation(
        ...
        dcr=args.dcr,
        dtp=args.dtp,
        phy_loss_model=args.phy_loss_model,
        capture_collision_model=args.capture_collision_model,
    )
