...
def tiles_for_bbox(bbox):
    ...
    names = []
    for lat_floor in range(math.floor(min_lat), math.floor(max_lat) + 1):
        for lon_floor in range(math.floor(min_lon), math.floor(max_lon) + 1):
    ...
    try:
        ...
        if download_path.suffix == ".gz":
            ...
            with zipfile.ZipFile(download_path) as archive:
                ...
                with archive.open(hgt_members[0]) as src, partial_hgt_path.open("wb") as out:
                    shutil.copyfileobj(src, out)
        else:
