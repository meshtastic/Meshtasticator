...
def apply_terrain_altitudes(conf, node_config):
    ...
    for node in node_config:
        node.position.z = grid.elevation_at(node.position.x, node.position.y) + node.antenna_height
    conf.TERRAIN_NODE_Z_IS_ABSOLUTE_ALTITUDE = True
...
def terrain_antenna_altitude(conf, grid, point):
    ...
    if getattr(conf, "TERRAIN_NODE_Z_IS_ABSOLUTE_ALTITUDE", False):
