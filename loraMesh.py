...
from lib.srtm import DEFAULT_SRTM_URL_TEMPLATE, terrain_grid_from_srtm
from lib.terrain import apply_terrain_altitudes, xy_to_latlon
...
""",
    ...
    if (
        ...
        parser.error("Incompatible argument selection. --from-file/--from-map/--preset and --router-type can not be used together")
    ...
    conf.TERRAIN_ENABLED = parsed_arguments.terrain_srtm
    conf.TERRAIN_GRID = None
    conf.TERRAIN_NODE_Z_IS_ABSOLUTE_ALTITUDE = False
    if parsed_arguments.terrain_profile_samples is not None:
        conf.TERRAIN_PROFILE_SAMPLES = parsed_arguments.terrain_profile_samples
    if parsed_arguments.terrain_srtm:
        try:
            conf.TERRAIN_GRID = terrain_grid_from_srtm(
                terrain_bbox,
